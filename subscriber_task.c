...
uint64_t subscribe_task_stack[SUBSCRIBE_TASK_STACK_SIZE / 8];

/* Task handle for this task. */
cy_thread_t subscriber_task_handle;

/* Handle of the queue holding the commands for the subscriber task */
cy_queue_t subscriber_task_q;
 ...
 *
 ******************************************************************************/
void subscriber_task(cy_thread_arg_t arg)
{
    ...
    while (true)
    {
        ...
        if(CY_RSLT_SUCCESS == cy_rtos_queue_get(&subscriber_task_q,&subscriber_q_data,portMAX_DELAY))
        {
